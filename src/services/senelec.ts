...
export interface SenelecPostpaidOptions {
...
export interface SenelecCalculationResult {
  ...
  // Senelec Postpaid period metadata
  nombre_jours?: number;
  limite_t1?: Decimal;
  limite_t2?: Decimal;
  seuil_tva?: Decimal;
...
export class SenelecPostpaidCalculator {
   ...
   * Uses 3 tranches + TVA 18% on consumption exceeding prorated TVA threshold.
   * Prorated dynamically according to billing period duration N days (default 60 days).
   */
  static async calculer(
    utilisateurId: string,
    consommation: number | string | Decimal,
    modePaiement: 'CASH' | 'DIGITAL',
    options?: SenelecPostpaidOptions
  ): Promise<SenelecCalculationResult> {
    const conso = MathUtils.toDecimal(consommation);
    if (conso.isNegative()) {
      throw new Error('La consommation ne peut pas être négative');
    ...
    // Determine duration N in days (defaulting to 60 days - standard Senelec bimonthly bill)
    ...
    const ratio = new Decimal(nbJours).dividedBy(30);
    ...
    let seuilTvaBase = new Decimal(250);
    let reductionT1 = new Decimal(0.10);

    for (const row of configRes.rows) {
      if (row.cle === 'senelec_seuil_tva') {
        seuilTvaBase = new Decimal(row.valeur);
      } else if (row.cle === 'senelec_reduction_t1') {
        reductionT1 = new Decimal(row.valeur);
      }
    }

    // Dynamic boundaries for N days
    const limitT1 = new Decimal(150).times(ratio); // 300 kWh for 60 days
    const limitT2 = new Decimal(250).times(ratio); // 500 kWh for 60 days
    const seuilTvaProrata = seuilTvaBase.times(ratio); // 500 kWh for 60 days
    const redevance = this.REDEVANCE_MONOPHASE.times(ratio); // 858 FCFA for 60 days
    ...
    if (conso.lte(limitT1)) {
      montantT1 = MathUtils.safeMultiply(conso, priceT1);
    } else if (conso.lte(limitT2)) {
      montantT1 = MathUtils.safeMultiply(limitT1, priceT1);
      montantT2 = MathUtils.safeMultiply(MathUtils.safeSubtract(conso, limitT1), priceT2);
    } else {
      montantT1 = MathUtils.safeMultiply(limitT1, priceT1);
      montantT2 = MathUtils.safeMultiply(MathUtils.safeSubtract(limitT2, limitT1), priceT2);
      montantT3 = MathUtils.safeMultiply(MathUtils.safeSubtract(conso, limitT2), priceT2);
    }

    const montantHt = MathUtils.safeAdd(MathUtils.safeAdd(montantT1, montantT2), montantT3);
    const taxeCommunale = MathUtils.safeMultiply(montantHt, this.TAXE_COMMUNALE_RATE);

    // TVA 18% applies only to HT revenue generated by consumption above seuilTvaProrata
    let htSubjectToTva = new Decimal(0);
    if (conso.gt(seuilTvaProrata)) {
      const excessConso = MathUtils.safeSubtract(conso, seuilTvaProrata);
    ...
    return {
      ...
      limite_t1: MathUtils.roundFinancial(limitT1),
      limite_t2: MathUtils.roundFinancial(limitT2),
      seuil_tva: MathUtils.roundFinancial(seuilTvaProrata),
